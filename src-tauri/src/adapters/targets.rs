// Compute target adapters (AD-6, Story 3.2): the boundary between the core
// and the machines jobs run on. Every job flows through the
// `ComputeTarget` trait — `submit(spec)`, `monitor(handle)`,
// `fetch(handle)` — resolved from a registry by adapter KIND. v1
// registers `Local` (argv-direct process execution; never `sh -c`);
// Story 3.3 registers the bare-SSH adapter behind the same trait, so
// nothing here may assume local execution.
//
...
// the SSH adapter's remote side will build a command line from these
// fields too.
...
pub enum TargetError {
    ...
    #[error("unknown_kind: `{0}` — no adapter of that kind is registered (v1: local)")]
    UnknownKind(String),
...
pub trait ComputeTarget: Send + Sync {
    ...
    /// `local` in v1; `ssh` registers in Story 3.3.
    fn kind(&self) -> &'static str;
    /// Submit a validated spec for execution. Re-validates the spec at the
    /// adapter boundary (AD-6); returns the handle monitor/fetch address.
    /// A process that fails to spawn is a job state, not a submit error —
    /// the handle comes back with the failure observable via `monitor`,
    /// so the lifecycle events always tell the story.
    fn submit(&self, spec: &JobSpec) -> Result<JobHandle, TargetError>;
...
/// The adapter registry (AD-6): kind → adapter. v1 registers `Local`; the
/// registration seam for the SSH adapter (Story 3.3) is `register` — the
/// trait and registry never change shape when it arrives. Named targets
...
impl TargetRegistry {
    /// The v1 registry: `local` registered; the SSH seam open.
    pub fn v1() -> Self {
    ...
    /// (bare-SSH) plug in here — one kind, one adapter.
    ...
    /// The registered kinds (v1: `local`) — what `target.declared` may name.
...
/// constructed shell string; AD-6). The child's captured output lands in
/// an in-memory slot a reaper task fills at exit, so `monitor` is a
/// non-blocking observation and `fetch` reads what was captured.
pub struct Local;

/// What the reaper task records when the child is done. `spawn_error` is
/// set when the process never came up (the handle still exists — the
/// lifecycle events carry the reason).
struct LocalExit {
    code: Option<i32>,
    stdout: String,
    stderr: String,
...
/// One live local job: the slot its reaper task fills at exit.
type LocalJobSlot = Arc<Mutex<Option<LocalExit>>>;

/// The Local adapter's live processes, keyed by handle id. Process-global
/// by design: children outlive any single command call; a restart loses
/// them (the log's read model keeps its last observed state honestly).
fn local_jobs() -> &'static Mutex<HashMap<String, LocalJobSlot>> {
    static JOBS: OnceLock<Mutex<HashMap<String, LocalJobSlot>>> = OnceLock::new();
...
impl ComputeTarget for Local {
    ...
    fn submit(&self, spec: &JobSpec) -> Result<JobHandle, TargetError> {
        ...
        let slot: Arc<Mutex<Option<LocalExit>>> = Arc::new(Mutex::new(None));
        match command.spawn() {
            Ok(child) => {
                ...
                tokio::spawn(async move {
                    ...
                    *exit_slot.lock().expect("local job exit slot poisoned") = Some(exit);
                });
            }
            // A spawn failure is the job's first (and terminal) state —
            // submit still succeeds, the failure surfaces through monitor
            // and lands as a reasoned `job.failed` (no silent ends).
            Err(e) => {
                *slot.lock().expect("local job exit slot poisoned") = Some(LocalExit {
                    code: None,
                    stdout: String::new(),
                    stderr: String::new(),
                    spawn_error: Some(e.to_string()),
        ...
        local_jobs()
            .lock()
            .expect("local job table poisoned")
            .insert(handle.id.clone(), slot);
        Ok(handle)
    }

    fn monitor(&self, handle: &JobHandle) -> Result<TargetJobStatus, TargetError> {
        let slot = lookup(handle)?;
        let guard = slot.lock().expect("local job exit slot poisoned");
        match guard.as_ref() {
            None => Ok(TargetJobStatus::Running),
            Some(exit) => Ok(observe(exit)),
        }
    }
...
fn lookup(handle: &JobHandle) -> Result<Arc<Mutex<Option<LocalExit>>>, TargetError> {
    local_jobs()
        .lock()
        .expect("local job table poisoned")
        .get(&handle.id)
...
fn observe(exit: &LocalExit) -> TargetJobStatus {
...
mod tests {
    ...
    fn the_registry_resolves_by_kind_with_a_typed_unknown() {
        ...
        assert_eq!(registry.kinds(), vec!["local"]);
        assert_eq!(registry.adapter("local").unwrap().kind(), "local");
        let err = match registry.adapter("ssh") {
            Err(e) => e,
            Ok(_) => panic!("`ssh` must not resolve in the v1 registry"),
        };
    ...
    async fn the_storys_freeform_spec_never_spawns_anything() {
        ...
        let err = local.submit(&spec("ls | rm -rf .", &[])).unwrap_err();
    ...
    async fn a_long_running_job_is_observed_running_then_terminal() {
        ...
        let handle = local.submit(&spec("sleep", &["1"])).unwrap();
    ...
    async fn a_job_captures_stdout_and_stderr() {
        ...
        let handle = local
            .submit(&spec("echo", &["pipeline | data ; $(not-a-command)"]))
            .unwrap();
    ...
    async fn a_nonzero_exit_is_a_reasoned_failure() {
        let local = Local;
        let handle = local.submit(&spec("sh", &["-c", "echo boom >&2; exit 3"])).unwrap();
        tokio::time::sleep(std::time::Duration::from_millis(300)).await;
        assert_eq!(
    ...
    async fn a_spawn_failure_is_a_handled_job_state_not_a_submit_error() {
        let local = Local;
        let handle = local.submit(&spec("definitely-not-a-binary-xyz", &[])).unwrap();
    ...
    async fn env_and_workdir_apply_to_the_spawned_process() {
        ...
        let handle = local.submit(&s).unwrap();
        tokio::time::sleep(std::time::Duration::from_millis(300)).await;
        let result = local.fetch(&handle).unwrap();
        assert_eq!(result.code, Some(0));
